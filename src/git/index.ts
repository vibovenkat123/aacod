...
${e.message}`);
    ...
    public clone(): Promise<void> {
        return new Promise(async (resolve, reject) => {
            ...
            const res = await execCmd(`git clone${this.opts.bare ? " --bare" : ""}${branch_cmd} ${this.opts.repo} ${this.opts.dest}`);
            if (res.err) {
                if (res.err.code === 127) {
...
export class GitConfig {
    ...
    public set(): Promise<void> {
        return new Promise(async (resolve, reject) => {
            const res = await execCmd(`git config ${this.opts.scope ? `--${this.opts.scope}` : '--system'} ${this.opts.name} ${this.opts.value}`)
            if (res.err) {
                if (res.err.code === 127) {
                    if (!this.opts.silent) Log.error(GIT_ERROR_MSG.GIT_CMD_NOT_FOUND)
                    reject(new GitError(res.err.message, GIT_ERROR_MSG.GIT_CMD_NOT_FOUND))
                    return
                }
